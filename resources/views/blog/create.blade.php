@extends('layouts.dashboard_app', ['activePage' => 'blogpost', 'titlePage' => __('Add blogpost')])
            ...
            <div class="col-lg-6 col-md-6 m-auto">
                <div class="card">
                    <div class="card-header bg-teal tx-white">
                        <h4 class="card-title tx-white">Add New blog post</h4>
                        <p class="card-category tx-white">new blogpost add form</p>
                    </div>
                    <div class="card-body">
                        ...
                        @endif
                        <form action=" {{ route('posts.store') }} " method="post" autocomplete="off" autofocus enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group {{ $errors->has('title') ? 'has-danger': ''}} ">
                                    <input type="text" name="title" id="input-title" class="form-control {{ $errors->has('title') ? 'is-invalid': ''}}" placeholder="{{__('Add Post Title')}}" required aria-required="true" value="{{old('title')}}">
                                    @if($errors->has('title'))
                            ...
                            <div class="col-sm-12">
                                <div class="form-group {{ $errors->has('content') ? 'has-danger': ''}} ">
                                    <textarea name="content" id="" cols="30" rows="10" class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" placeholder="{{__('Enter Post Message/Content')}} " required=true aria-required="true">{{old('content')}} </textarea>
